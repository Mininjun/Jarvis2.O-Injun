...
# https://readloud.net/english/american/female-voice-JennyNeural.html
# Required packages (install once):
# pip install pygame edge-tts python-dotenv
...
async def TextToAudioFile(text) -> None:
    file_path = fr"Data\speech.mp3" # Define the path where the speech file will be saved
    ...
    await communicate.save(fr"Data\speech.mp3") # Save the generated speech as an MP3 file
...
def TTS(Text, func=lambda r=None: True):
    while True:
        try:
            ...
            pygame.mixer.music.load(fr"Data\speech.mp3")
            pygame.mixer.music.play() # Play the audio

            # Loop until the audio is done playing or the function stops
            while pygame.mixer.music.get_busy():
                if not func() == False: # Check if the external function returns False
                    break
            ...
            try:
                # Call teh provided function with False to signal the end of TTS
                func(False)
